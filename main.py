...
@bot.user_command(
    guild_ids=[GUILD_ID])  # create a user command for the supplied guilds
...
@commands.cooldown(
    1, 5,
    commands.BucketType.user)  # the command can only be used once in 5 seconds
async def server(
  ...
  if action == "start":
    try:
      ...
      instance_state = response[response_root_field][0][response_state_field][
          'Name']

      await ctx.respond(
          embed=embed_maker(["Instance", action.capitalize()], action,
                            [instance_id, instance_state], thumb_url, True,
                            "discord-bot (c)", 0x18ec5f))
...
async def fetch_product_url():
  url = 'https://api.store.nvidia.com/partner/v1/feinventory?skus=NVGFT470S&locale=nl-nl'
  timeout = aiohttp.ClientTimeout(
      total=10)  # Set a 10-second timeout for the whole operation
  async with aiohttp.ClientSession(
      timeout=timeout) as session, session.get(url) as response:
...
async def check_product_url():
  data = await fetch_product_url()
  print(f"{data = }")
  if data['listMap'][0]['product_url'] != "":
    channel = bot.get_channel(853940969518268416)
    if channel:
      await channel.send(
          "Looks like NVIDIA GeForce RTX 4070 SUPER is back in stock in NL!. @yen1co Proceed at <https://store.nvidia.com/nl-nl/geforce/store/?page=1&limit=9&locale=nl-nl&manufacturer=NVIDIA&gpu=RTX%204070%20SUPER>"
      )
  else:
    print("Product URL is empty.")
...
async def on_ready():
  ...
  # change_time.start()
  check_product_url.start()  # Start the periodic task
